...
// @LINE:90
def getMostKPopularDatasets(k:Integer): Call = {
   import ReverseRouteContext.empty
   Call("GET", _prefix + { _defaultPrefix } + "dataset/getMostKPopularDatasets/" + implicitly[PathBindable[Integer]].unbind("k", k))
}
...
def getMostKPopularDatasets : JavascriptReverseRoute = JavascriptReverseRoute(
   ...
   """
      function(k) {
      return _wA({method:"GET", url:"""" + _prefix + { _defaultPrefix } + """" + "dataset/getMostKPopularDatasets/" + (""" + implicitly[PathBindable[Integer]].javascriptUnbind + """)("k", k)})
      }
   """
)
...
// @LINE:90
def getMostKPopularDatasets(k:Integer): play.api.mvc.HandlerRef[_] = new play.api.mvc.HandlerRef(
   play.api.Play.maybeApplication.map(_.global).getOrElse(play.api.DefaultGlobal).getControllerInstance(classOf[controllers.DatasetController]).getMostKPopularDatasets(k), HandlerDef(this.getClass.getClassLoader, "", "controllers.DatasetController", "getMostKPopularDatasets", Seq(classOf[Integer]), "GET", """""", _prefix + """dataset/getMostKPopularDatasets/$k<[^/]+>""")
)
